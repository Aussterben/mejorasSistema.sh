...
# Recuerda dar permisos de ejecución con "chmod +x betterSystem.sh"
...
opciones(){
  case $1 in 
    ...
    -e)
      clear
      echo -e "\n"${yellowColour}[+]${endColour}${grayColour} Descargando herramientas extras, espere, por favor...${endColour}
      sleep 1
      sudo apt install cmatrix -y &>/dev/null
      sleep 1
      sudo apt install docker.io -y &>/dev/null
      sleep 1
      sudo apt install htop -y &>/dev/null
      sleep 1
      sudo apt install btop -y &>/dev/null
      sleep 1
...
⠀⠀⠀⠀⠈⠉⠑⠒⠒⠂⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠤⠒⠒⠒⠊⠉⠁⠀⠀⠀⠀
      ...
      echo -e "\n\t${yellowColour}[!]${endColour}${grayColour} Version:${endColour}${redColour}2.0${endColour}"
      ;;
  esac
}
